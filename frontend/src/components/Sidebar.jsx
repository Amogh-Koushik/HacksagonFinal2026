...
import { LayoutDashboard, UserPlus, ClipboardList, LogOut, Shield, X } from 'lucide-react'
    ...
    <>
      {mobileOpen && (
        <button
          type="button"
          onClick={closeMobile}
          className="lg:hidden fixed inset-0 bg-slate-950/45 backdrop-blur-[1px] z-40"
          aria-label="Close navigation menu"
        />
      )}

      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 lg:z-auto w-[280px] lg:w-[264px] bg-slate-900 flex flex-col shrink-0 border-r border-slate-800 transform transition-transform duration-300 ${
        ...
        <div className="p-6 border-b border-slate-800">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={closeMobile}
              className="lg:hidden w-8 h-8 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-800 flex items-center justify-center"
